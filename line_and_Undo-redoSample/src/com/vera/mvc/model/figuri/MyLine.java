...
public class MyLine implements FigureInterface, Serializable {
    Line2D.Double myFigure;
    public MyLine(){
        myFigure = new Line2D.Double();
    }
    @Override
    public void setFrameFromDiagonal(double x1, double y1, double x2, double y2) {
        myFigure.setLine(x1,y1,x2,y2);
    }

    @Override
    public void setFrameFromDiagonal(Point2D first, Point2D second) {
        myFigure.setLine(first,second);
    }

    @Override
    public FigureInterface clone(){
        MyLine copy = new MyLine();
        copy.myFigure = (Line2D.Double) myFigure.clone();
        return copy;
    }

    @Override
    public double getMinX() {
        return myFigure.getX1();
    }

    @Override
    public double getMinY() {
        return myFigure.getY1();
    }

    @Override
    public double getMaxX() {
        return myFigure.getX2();
    }

    @Override
    public double getMaxY() {
        return myFigure.getY2();
    }
